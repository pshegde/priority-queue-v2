...
describe('test', () => {
  ...
  describe('test with duplicates allowed', () => {
    it('should allow to insert a duplicate element', () => {
      let obj = new PriorityQueue(Box.prototype.comparator, true)
      obj.queue(new Box(1, 5))
      obj.queue(new Box(5, 5))
      obj.queue(new Box(5, 5))
   
      assert.equal(obj.dequeue().area, 25)
      assert.equal(obj.dequeue().area, 25)
      assert.equal(obj.dequeue().area, 5)
    })
  })
})
